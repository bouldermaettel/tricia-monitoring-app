...
function getMatrixValues(section) {
...
function matrixToImageDataUrl(section) {
    ...
    for (let i = 0; i < allValues.length; i += 1) {
        ...
        for (let j = 0; j < allValues.length; j += 1) {
            ...
            const color = expected === observed ? '#DCFCE7' : expected > observed ? '#FEE2E2' : '#FEF3C7';
            const textColor = expected === observed ? '#166534' : expected > observed ? '#991B1B' : '#92400E';
            ctx.fillStyle = color;
...
function buildReportHtml({ generatedAt, filters, matrixSections, tableColumns, tableRows, matrixImageData, }) {
...
export function MatrixReportExportButton({ fileNamePrefix, generatedAt, filters, matrixSections, tableColumns, tableRows, }) {
    ...
    return (_jsxs("div", { className: "flex gap-2", children: [_jsxs("button", { onClick: exportHtml, disabled: disabled || isGenerating, className: "flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-stone-600 border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors", children: [_jsx(FileText, { size: 14 }), isGenerating ? 'Generating...' : 'Report HTML'] }), _jsxs("button", { onClick: exportPdf, disabled: disabled || isGenerating, className: "flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-stone-600 border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors", children: [_jsx(Download, { size: 14 }), isGenerating ? 'Generating...' : 'Report PDF'] })] }));
}
