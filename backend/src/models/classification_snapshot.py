...
class ClassificationSnapshot(Base):
    ...
    user_p: Mapped[int | None] = mapped_column(Integer, default=1, nullable=True)
    user_d: Mapped[int] = mapped_column(Integer)
    tri_risk: Mapped[int | None] = mapped_column(Integer, nullable=True)
    wimi_risk: Mapped[int | None] = mapped_column(Integer, nullable=True)
